...
{
  if (cbxFont->Text == "") {
    Application->MessageBox(L"フォントを指定してください。", CASSAVA_TITLE, 0);
    return;
  }
...
void TfmLetter::DataSetDefault()
{
  ...
  if (cbxFont->ItemIndex < 0) {
    int hg = cbxFont->Items->IndexOf(L"@HG正楷書体-PRO");
    if (hg >= 0) {
      cbxFont->ItemIndex = hg;
      return;
    }
    int bizUd = cbxFont->Items->IndexOf(L"@BIZ UDP明朝 Medium");
    if (bizUd >= 0) {
      cbxFont->ItemIndex = bizUd;
      return;
    }
    int msp = cbxFont->Items->IndexOf(L"ＭＳ Ｐ明朝");
    if (msp >= 0) {
